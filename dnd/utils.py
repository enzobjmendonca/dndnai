...
0.  **Information Gathering (`get_all_players`, `get_all_locations`, `get_all_npcs`):**
...
1.  **Player State Changes (`update_player_state`):**
    ...
    *   **ACTION:** MUST call `update_player_state` with the **complete and updated** player data structure. Provide *all* fields, ensuring data fetched via `get_all_players()` is correctly modified. Append new spoken dialogue to the *end* of the `dialogue` list. `{PLAYER_EXAMPLE}` structure must be followed (ensure schema includes HP, inventory, status, dialogue list, etc.).
...
3.  **Existing Location Changes (`update_location_state`):**
    ...
    *   **ACTION:** MUST call `update_location_state` with the location's **complete and updated** data. Fetch current state via `get_all_locations()` first if needed. Provide *all* fields according to the `{LOCATION_EXAMPLE}` structure.
...
5.  **Existing NPC Changes (`update_npc_state`):**
    ...
    *   **ACTION:** MUST call `update_npc_state` with the NPC's **complete and updated** data. Fetch current state via `get_all_npcs()` first. Provide *all* fields. Append new spoken dialogue to the *end* of the `dialogue` list. `{NPC_EXAMPLE}` structure must be followed.
...
6.  **Dice Rolls (`roll_dice`):**
    *   **TRIGGER:** When an action's success or an event's magnitude is uncertain and requires randomization based on game rules. **This explicitly includes:**
        *   **Player initiating an attack against ANY NPC:** Request an attack roll (e.g., `roll_dice(dice_string="d20+<PlayerAttackBonus>")`). You MUST determine the `<PlayerAttackBonus>` from data retrieved via `get_all_players()`.
        *   **Calculating damage after a successful hit:** Request a damage roll (e.g., `roll_dice(dice_string="<PlayerDamageDice>+<PlayerDamageBonus>")`). Determine dice/bonus from `get_all_players()`.
        *   **NPC actions in combat:** Request attack and damage rolls for NPCs. Determine bonuses/dice from `get_all_npcs()`.
    ...
    *   **INTERACTION:** You will call the tool. The system will execute the roll and provide the numerical result back to you. You then use this result in your narrative and subsequent logic (e.g., compare attack roll result to NPC AC retrieved via `get_all_npcs()`).
...
2.  **(Function Call):** `get_all_npcs()` (to get Bob's data including AC)
3.  **(Function Call):** `get_all_players()` (to get Player's data including attack bonus)
...
14. **(Function Call):** `get_all_players()` (for Player AC)
15. **(Function Call):** `get_all_npcs()` (for Bob's attack bonus/damage - may already have from step 10)
